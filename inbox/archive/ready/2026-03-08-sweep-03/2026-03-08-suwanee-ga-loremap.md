...
**Excerpt note:** The Phase II revelation line is the strongest self-representing sentence and carries the terrain logic cleanly.

**Theme notes:** The raw classification labels are out-of-registry and were not carried forward as themes. Intake mapped this to `place`, `observation`, `signal`, and `transmission` because the document is fundamentally about reading a location through recurring environmental cues and carried messages.

**Constellation notes:** No active constellation is an exact match. This reads like place-lore rather than pressure, maintenance, or artifact recovery; leave constellation unassigned unless a larger place-based cluster emerges.

**Suggested new fields flagged:** no

**Ambiguities for human review:**
  - This sits on the boundary between `scroll` and `codex`; it was classified as `scroll` because the authored text is the primary experience, not a reusable system specification.
  - Confirm whether the "Codex classification" line should remain in the body or move into supporting presentation if promoted.
  - Companion file `inbox/drop/loremap, suwanee/Read_me_object-orientation.md` appears to be contextual guidance rather than a complete standalone object.

**Suggested related objects (by description, not id):**
  - A field log about North Georgia terrain, fog, and return-path observation.
  - A scroll about a ridge line holding even when it cannot be seen.
  - A scroll arguing that signal emerges through ongoing acts of noticing.

**Source files:**
  - `inbox/drop/loremap, suwanee/LOREMAP_Suwanee_GA.md`
  - `inbox/drop/loremap, suwanee/Read_me_object-orientation.md`
